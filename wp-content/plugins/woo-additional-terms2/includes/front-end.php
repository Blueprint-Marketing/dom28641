...
if (!function_exists('woo_additional_terms_checkout_checkbox')): 
	function woo_additional_terms_checkout_checkbox() {
		...
		$woo_additional_terms_notice_contents_agreement = ( get_option( 'woo_additional_terms_title_contents_agreement' ) ) ? esc_attr( get_option( 'woo_additional_terms_title_contents_agreement' ) ) : '';
		...
		if( isset($woo_additional_terms_notice) && !empty($woo_additional_terms_notice) ):
			...
			if( isset($woo_additional_terms_notice_contents_agreement) && !empty($woo_additional_terms_notice_contents_agreement) ):
					echo '<p class="form-row terms">' . PHP_EOL;
					echo '<input type="checkbox" class="input-checkbox" name="woo-additional-terms-contents-checkbox" id="woo-additional-terms-contents-checkbox" />' . PHP_EOL;
					echo '<label for="woo-additional-terms-contents-checkbox" class="checkbox">' . str_replace( '%link%', '<a href="' . get_permalink($woo_additional_terms_page) . '" target="_blank">' . $woo_additional_terms_title . '</a>', $woo_additional_terms_notice_contents_agreement ) . ' <span class="required">*</span></label>' . PHP_EOL;
					echo '</p>' . PHP_EOL;
			endif;
		endif;
...
if (!function_exists('woo_additional_terms_checkout_notice_error')): 
	function woo_additional_terms_checkout_notice_error() {
		// Retrieve plugin option value(s)
		$woo_additional_terms_notice = ( get_option( 'woo_additional_terms_notice' ) ) ? esc_attr( get_option( 'woo_additional_terms_notice' ) ) : '';
		$woo_additional_terms_title_contents_agreement = ( get_option( 'woo_additional_terms_title_contents_agreement' ) ) ? esc_attr( get_option( 'woo_additional_terms_title_contents_agreement' ) ) : '';
	    ...
	    if ( !isset($woo_additional_terms_notice, $_POST['woo-additional-terms-checkbox']) && !empty($woo_additional_terms_notice)) {	
		 ...
		 if ( !isset($woo_additional_terms_notice_error, $_POST['woo-additional-terms-contents-checkbox']) && !empty($woo_additional_terms_notice_error)) {	
	    	wc_add_notice( $woo_additional_terms_notice_error, 'error' );
	    }
