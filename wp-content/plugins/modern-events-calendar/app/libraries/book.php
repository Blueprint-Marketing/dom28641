...
class MEC_book extends MEC_base
{
    ...
    public function get_tickets_availability($event_id, $date)
    {
        ...
        // No Ticket Found!
        if(!is_array($availability) or (is_array($availability) and !count($tickets))) return $availability;
    ...
    public function get_invoice_link($transaction_id)
    {
        ...
        $url = $main->add_qs_var('id', $transaction_id, $url);

        return wp_nonce_url($url, 'mec_invoice_download');
    }
}