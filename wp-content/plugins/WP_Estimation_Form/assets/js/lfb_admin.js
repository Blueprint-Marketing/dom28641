...
function lfb_addLinkInteraction(data) {
    ...
    jQuery.each(lfb_steps, function () {
        var step = this;
        // $select.append('<option value="' + step.id + '_stepqt" data-variable="numberfield" data-type="step">' + step.title + ' :  ' + lfb_data.texts['totalQuantity'] + ' </option>');
        ...
        success: function (rep) {
            ...
            lfb_logsTable = jQuery('#lfb_logsTable').DataTable({
                ...
                'language': {
                    ...
                    'infoEmpty': lfb_data.texts['noRecords'],
...
function lfb_confirmModifyTotal() {
    ...
    if (total > 0 && totalSub > 0) {
        jQuery('.lfb_logEditorContainer #lfb_summaryTotal').html('<span>' + summaryPrice + '</span>' + lfb_currentLogSubTxt + ' <br/>+' + summaryPriceSingle);
    } else if (totalSub > 0) {
        jQuery('.lfb_logEditorContainer #lfb_summaryTotal').html('<span>' + summaryPrice + '</span>' + lfb_currentLogSubTxt);
    } else if (total > 0) {
        jQuery('.lfb_logEditorContainer #lfb_summaryTotal').html('<span>' + summaryPriceSingle + '</span>');
    }
}
