...
class LFB_Core {
    ...
    private function generateItemHtml($dataItem, $form, $dataSlide, $isFinalStep) {
        ...
        if ($chkDisplay) {
            ...
            if ($dataItem->type == 'picture') {
                ...
                $response .= '<div class="lfb_richtext lfb_item" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->richtext) . '</div>';
            } else if ($dataItem->type == 'shortcode') {
                $response .= '<div class="lfb_richtext lfb_item" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->shortcode) . '</div>';
            ...
            if (count($formReq) > 0) {
                ...
                if ($form->use_stripe && $form->paymentType != "email") {
                  ...
                  <div class="form-group">
                    ...
                    $response .= '<span class="payment-errors" style="color:red; font-size: 20px;padding-top: 28px;"></span>
                  </div>';
    ...
    private function lfb_generatePdfCustomer($order, $form) {
        $contentPdf = '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"/><style>body,*{font-family: "dejavu sans" !important; } hr{color: #ddd; border-color: #ddd;} table{width: 100% !important; line-height: 18px;} table td, table th{width: auto!important; border: 1px solid #ddd; line-height: 16px;overflow-wrap: break-word;}table td,table tbody th  {padding-top: 2px !important; padding-bottom: 6px !important} table thead th {padding: 8px;line-height: 18px;}</style></head><body>' . $order->pdfContentUser . '</body></html>';
        ...
        if (!$form->useCaptcha || ($_SESSION['lfb_random_number'] != "" && strtolower($_SESSION['lfb_random_number']) == strtolower($_POST['captcha']))) {
            ...
            if (!isset($_POST['gravity']) || $_POST['gravity'] == 0) {
                ...
                if ($_POST['email_toUser'] == '1') {
                    ...
                    $content = $this->prepareEmailContent($content,$form);                  
                ...
                $contentAdmin = $this->prepareEmailContent($content,$form);    
                ...
                $wpdb->insert($table_name, array('ref' => $form->ref_root . $current_ref, 'email' => $email, 'phone' => $phone, 'firstName' => $firstName, 'lastName' => $lastName,
                    ...
                    'paymentKey' => $paymentKey, 'finalUrl' => $finalUrl));
    ...
    private function prepareEmailContent($content,$form){
