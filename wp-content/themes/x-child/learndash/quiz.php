...
if ( $show_content ) {
    ...
    if ( $attempts_left ) {
		...
		?>
			<p class="learndash_already_taken"><?php echo sprintf( esc_html_x( 'You have already taken this %1$s %2$d time(s) and may not take it again.', 'placeholders: quiz, attempts count', 'learndash' ), LearnDash_Custom_Label::label_to_lower('quiz'), $attempts_count ); ?></p>
            
            <?php 
            if( function_exists( 'FHS_Quiz_Retake_Course_UI' ) ){
                FHS_Quiz_Retake_Course_UI( $course_id, $user_id );
            }
            ?>
            
            <p id="learndash_already_taken" style="margin:0"></p>
		<?php
    }
}