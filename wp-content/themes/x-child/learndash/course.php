...
<?php if ( $logged_in ) : ?>
    ...
    <?php if( learndash_get_setting( $post->ID, 'expire_access' ) ):?>
        <?php
        if( function_exists( 'fhs_certificates' ) ){
            fhs_certificates()->c->fhs_certificates_manager->print_certificates_list( $post->ID, $user_id );
        }
        ?>
    <?php else: ?>
        <?php  if ( ! empty( $course_certficate_link ) ) : ?>
            <div id="learndash_course_certificate" class="learndash_course_certificate">
                <?php /*<a href='https://foodhandlersolutions.com/certificates/food-handler-certificate/' class="btn-blue" target="_blank"><?php echo apply_filters('ld_certificate_link_label', __( 'PRINT YOUR CERTIFICATE', 'learndash' ), $user_id, $post->ID ); ?></a>*/?>
...
if( $expires_on ){
    //echo "<br><br>Course access expires on: " . date( get_option( 'date_format' ), $expires_on ) . "<br><br>";
}
?>
		...
		if ( $show_course_content ) :
            ...
            <div id="learndash_course_content" class="learndash_course_content">
                        ...
                        <div id="quiz_list" class=“quiz_list”>
                            ...
                            <?php foreach( $quizzes as $quiz ) :
                                if ( function_exists( 'FHS_Quiz_get_status' ) ) {
                                    $quiz['status'] = ( FHS_Quiz_get_status( $quiz['post']->ID ) ) ? 'completed' : 'notcompleted';
                                }?>
				...
				if( function_exists( 'FHS_Quiz_Retake_Course_UI' ) ){
					FHS_Quiz_Retake_Course_UI( $post->ID, $user_id );
				}
				?>
